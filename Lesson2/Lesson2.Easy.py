...
# Можно решить эту задачу и без буфера, перебирая неизменяемый список удаляемых значений в цикле for, а список из которого удаляются в цикле while,
# проверяя на наличие совпадения со списком на удаление и делая remove(), пока все вхождения не будут удалены, но имхо это потребует больше ресурсов
